...
class DieselMQTTSubscriber:
    def __init__(self, client: mqtt.Client, plant_id: str):
        ...
        # 전체 디젤 디바이스의 명령을 와일드카드로 수신 (+ 사용)
        self.command_topic = f"{plant_id}/diesel/+/command"
        self.on_command_callback: Optional[Callable[[str, dict], None]] = None
        # command 수신을 EMS 통신 생존 증거로 활용 (rule-engine-spec §5.4 COMMS_TIMEOUT)
    ...
    def _on_message(self, client, userdata, msg):
        try:
            # 토픽에서 device_id 파싱: PLANT-ALPHA/diesel/diesel-01/command -> diesel-01
